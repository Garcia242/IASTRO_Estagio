...
model = CmdStanModel(
    stan_file="Model4/Model/together.stan",                         # Stan model file location
    cpp_options={
...
fit = model.sample(
    data="Model4/Data/3together.json",                                # the location of the data file
    output_dir="output4/together",
    ...
    inits={ "H0": normal (loc= 70, scale=10),"Om": normal(loc=0.3, scale=0.1), "lambda":1},  # initial values for each parameter
...
az.plot_trace(posterior, var_names=( "H0",'Om', "lambda"), compact=False, combined=False)
plt.tight_layout()
plt.show()

# show corner plot
samples = [fit.stan_variable("H0"), fit.stan_variable("Om"), fit.stan_variable("lambda")]
mcsamples = MCSamples(samples=samples, names=[ "H0", "Om", "lambda"], labels=["H0","Om", "lambda"])
