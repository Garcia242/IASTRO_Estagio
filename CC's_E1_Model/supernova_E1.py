...
model = CmdStanModel(
    stan_file="model/supernova_E1.stan",                         # Stan model file location
    cpp_options={'STAN_THREADS': 'TRUE', 'STAN_CPP_OPTIMS': 'TRUE'}   # a few optimizations, feel free to ignore this
)

# configure and fit the model
fit = model.sample(
    data="data/snia.json",                                # the location of the data file
    output_dir="output_mod1/SNIa",
    ...
    inits={"H0": normal(loc=70, scale=10), "Om": normal(loc=0.3, scale=0.1), "zeta": normal(loc=0.3, scale=0.1)},  # initial values for each parameter
