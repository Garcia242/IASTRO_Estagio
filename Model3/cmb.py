...
model = CmdStanModel(
    stan_file="Model3/Model/cmb.stan",                         # Stan model file location
    cpp_options={
...
fit = model.sample(
    ...
    inits={"H0": normal(loc=70, scale=10), "Om": normal(loc=0.3, scale=0.1), "Omb": normal(loc=0.05, scale=1)},  # initial values for each parameter
...
az.plot_trace(posterior, var_names=('H0', 'Om', "Omb"), compact=False, combined=False)
plt.tight_layout()
plt.show()
