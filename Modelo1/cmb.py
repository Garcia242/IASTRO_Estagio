...
model = CmdStanModel(
    stan_file="Modelo1/Model/cmb.stan",                         # Stan model file location
    cpp_options={
...
fit = model.sample(
    data="Modelo1/Data/cmb.json",                                # the location of the data file
    output_dir="output1/cmb",
    ...
    inits={"H0": normal(loc=70, scale=10), "Om": 0.3,"Omb": 0.05, "zeta": 1},  # initial values for each parameter
...
az.plot_trace(posterior, var_names=('H0', 'Om',"Omb", "zeta"), compact=False, combined=False)
plt.tight_layout()
plt.show()

# show corner plot
samples = [fit.stan_variable("H0"), fit.stan_variable("Om"),fit.stan_variable("Omb"), fit.stan_variable("zeta")]
mcsamples = MCSamples(samples=samples, names=["H0", "Om","Omb", "zeta"], labels=["H0", "Om","Omb", "zeta"])
